...
class Block1(Block):
    ...
    def process_input(self, events, pressed_keys, pressed_mouse):
        ...
        if pressed_keys[py.K_LEFT]:
            self.direction.x = -1
        if not pressed_keys[py.K_LEFT] and not pressed_keys[py.K_RIGHT]:
            self.direction.x = 0

        if pressed_keys[py.K_UP] and self.on_ground:
            self.jump()
    ...
    def horizontal_movement_collision(self, objs: list):
        ...
        for obj in objs:
            if obj.rect.colliderect(self.rect):
                if type(obj) == Wall or type(obj) == PurpleDoor or type(obj) == BlueDoor:
